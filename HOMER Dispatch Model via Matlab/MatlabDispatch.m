...
%%         
      ...
      if battery_max_discharge_power >= matlab_simulation_variables.net_load 
          ...
          if simulation_parameters.has_converter == true
              ...
              (min(matlab_simulation_variables.net_load, simulation_parameters.converters(1).inverter_capacity - simulation_state.converters(1).inverter_power_output) ...
              + simulation_state.converters(1).inverter_power_output);
          else
              parameters(option_1,inverter_power_output) = 0;
          end
          ...
          if simulation_parameters.has_converter == true
              ...
              parameters(option_1,battery_power_setpoint) = 0;
          end          
         ...
         if simulation_parameters.has_converter == true
          ...
          if simulation_parameters.has_converter == true
              parameters(option_1,inverter_power_output) = ...
              (min(battery_max_discharge_power, simulation_parameters.converters(1).inverter_capacity - simulation_state.converters(1).inverter_power_output)...
              + simulation_state.converters(1).inverter_power_output);
          else
              parameters(option_1,inverter_power_output) = 0;
          end
          ...
          if remaining_load > min_load % we run generator at either remaining load or full capacity
            ...
            parameters(option_1,ac_bus_excess_electricity) = ...
                 ...
                 max(0, parameters(option_1,generator_power_setpoint)-matlab_simulation_variables.net_load);
             % ac bus load served
             parameters(option_1,ac_bus_load_served) = parameters(option_1,primary_load_served);
             % ac bus operating capacity requested: reflect requested load reserve
             parameters(option_1,ac_bus_capacity_requested) = ...
 ...
 %%         
      ...
      if matlab_simulation_variables.net_load <= min_load
          ...
          parameters(option_2,ac_bus_capacity_shortage) = ...
              max(0,parameters(option_2,ac_bus_capacity_requested) - parameters(option_2,ac_bus_operating_capacity_served));
          ...
          if simulation_parameters.has_converter == true
              ...
              parameters(option_2,battery_power_setpoint) = 0;
          end
          % primary load served
...
%%          
      ...
      if generator_max_possible_output_from_load_and_battery <= min_load
          ...
          parameters(option_3,ac_bus_excess_electricity) = excess_solar_step_one + ...
              excess_generator_output - parameters(option_3,rectifier_power_input);
          % battery net charge
          ...
          if remain_load>0 % we run generator at full capacity with battery assistance, this is equal to option 2
           ...
           if simulation_parameters.has_converter == true
            ...
            if simulation_parameters.has_converter == true
                ...
                parameters(option_3,battery_power_setpoint) = 0;
            end
            % primary load served
            ...
            parameters(option_3,ac_bus_excess_electricity) = ...
                excess_solar_step_one + excess_generator_output - parameters(option_3,rectifier_power_input); 
          end
          ...
          parameters(option_3,ac_bus_capacity_shortage) = ...
              max(0, parameters(option_3,ac_bus_capacity_requested) - parameters(option_3,ac_bus_operating_capacity_served));    
      end
